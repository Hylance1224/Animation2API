...
def store_page_file():
    ...
    with open(path + ss + ".dom", "w", encoding='utf-8') as f:  # 设置文件对象
...
# 移除一些可能是广告，重复的，非当前app的component
...
def store_operate(node, path, activity):
    ss = str(global_num)
    f = open(path + ss + "operate.txt", "w", encoding='utf-8')   # 设置文件对象
    if is_operated(node):
        f.write('True')
    else:
        f.write('False') #第一行，true或false
    f.write('\n')
    f.write(node.getAttribute('class'))  # 第二行，类型
    f.write('\n')
    f.write(node.getAttribute('text'))  # 第三行，text
    f.write('\n')
    f.write(node.getAttribute('content-desc'))  # 第四行，content-desc
    f.write('\n')
    f.write(GetFeatures.get_sibling_text(node))  # 第五行，邻居节点
    f.write('\n')
    f.write(GetFeatures.get_child_content(node))  # 第6行，子节点
    f.write('\n')
    f.write(node.getAttribute('resource-id'))  # 第7行，id
    f.write('\n')
    f.write(activity)  # 第8行，activity
    f.close()
...
def traverse(package_name, store_path):
    ...
    while(1):
        ...
        if node == 'blank':
            ...
            # print(is_operated(node))
            # operate_node(node, store_path)
            threads = []
...
if __name__ == "__main__":
    ...
    # path = 'G:/apk_used_for_animation/apk_photo'
    p = 'G:/apk/apk_photo'
    static_id_path = 'G:/apk/apk_photo/API/'
    # 开始一个新APP的浏览
    for i in range(773, 800): #558到600的apk没有  694
        print(i)
